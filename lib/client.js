window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var buttonStyle = { background: "transparent", color: "var(--dsw-alias-label-secondary)", border: "1px solid var(--dsw-alias-border-l2)", borderRadius: "6px", fontSize: "12px", lineHeight: "16px", padding: "1px 8px", cursor: "pointer", fontFamily: "inherit" };
		var numberStyle = { background: "var(--dsw-alias-bg-layer-1)", color: "var(--dsw-alias-label-primary)", border: "1px solid var(--dsw-alias-border-l2)", borderRadius: "6px", fontSize: "12px", lineHeight: "16px", padding: "1px 6px", width: "56px", fontFamily: "inherit" };
		// ---- component ----
		function BalanceDock() {
			...
			// control: preset select + (custom seconds input) + refresh button
			var presetValue = presetValueOf(config);
			var options = [];
			for (var j = 0; j < PRESETS.length; j++) {
				var p = PRESETS[j];
				options.push(React.createElement("option", { key: String(p.value), value: String(p.value) }, p.label));
			}
			var selectEl = React.createElement("select", {
				...
				onChange: function (event) {
					...
					else if (value === "custom") setConfig({ mode: "auto", seconds: config.seconds });
			...
			if (presetValue === "custom") {
				inputEl = React.createElement("input", {
					...
					title: "刷新间隔（秒，" + MIN_SECONDS + "–" + MAX_SECONDS + "）",
					onChange: function (event) {
						setConfig({ mode: "auto", seconds: clampSeconds(event.target.value) });
					}
				});
			}
			...
			var children = [selectEl];
			if (inputEl !== null) children.push(inputEl);
			children.push(refreshButton);
			children.push(React.createElement("span", { key: "sep", style: mutedStyle }, "|"));
